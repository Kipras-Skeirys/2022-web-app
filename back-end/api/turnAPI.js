...
const turnCoordinates = [
    { coordinates: [ 40.7128, -74.0060 ], ice: { stun: [ 'stun:coturn.nyc3.demo.com' ], turn: [ 'turn:coturn.nyc3.demo.com:3478?transport=udp', 'turn:coturn.nyc3.demo.com:3478?transport=tcp' ] } },
    { coordinates: [ 37.6192, -122.3816 ], ice: { stun: [ 'stun:coturn.sfo3.demo.com' ], turn: [ 'turn:coturn.sfo3.demo.com:3478?transport=udp', 'turn:coturn.sfo3.demo.com:3478?transport=tcp' ] } },
    { coordinates: [ 50.0380, 8.5622 ], ice: { stun: [ 'stun:coturn.fra1.demo.com' ], turn: [ 'turn:coturn.fra1.demo.com:3478?transport=udp', 'turn:coturn.fra1.demo.com:3478?transport=tcp' ] } },
    { coordinates: [ 51.5072, -0.1276 ], ice: { stun: [ 'stun:coturn.lon1.demo.com' ], turn: [ 'turn:coturn.lon1.demo.com:3478?transport=udp', 'turn:coturn.lon1.demo.com:3478?transport=tcp' ] } },
    { coordinates: [ 1.3521, 103.8198 ], ice: { stun: [ 'stun:coturn.sgp1.demo.com' ], turn: [ 'turn:coturn.sgp1.demo.com:3478?transport=udp', 'turn:coturn.sgp1.demo.com:3478?transport=tcp' ] } },
    { coordinates: [ 12.9716, 77.5946 ], ice: { stun: [ 'stun:coturn.blr1.demo.com' ], turn: [ 'turn:coturn.blr1.demo.com:3478?transport=udp', 'turn:coturn.blr1.demo.com:3478?transport=tcp' ] } },
    { coordinates: [ -33.8688, 151.2093 ], ice: { stun: [ 'stun:coturn.syd1.demo.com' ], turn: [ 'turn:coturn.syd1.demo.com:3478?transport=udp', 'turn:coturn.syd1.demo.com:3478?transport=tcp' ] } }
]

module.exports.getIceCredentials = function(country_code) {
    return new Promise((promiseResolve, promiseReject) => {

        // [ ] Check time
        let ttl = parseInt(Date.now() / 1000) + 60
        let credential
        let username = [ttl, 'test'].join(':')
        let hmac = crypto.createHmac('sha1', process.env.TURN_SECRET)
        ...
        if (!country_code || country_code === 'T1' || country_code === 'XX' || !Object.hasOwn(countryCoordinates, country_code)) {
            // Return full turn/stun list
            turnCoordinates.map((turn) => {
                servers.iceServers[0].urls.push(...turn.ice.stun)
                servers.iceServers[1].urls.push(...turn.ice.turn)
            })
            //console.log(`[NOTICE] [unknown country_code] country_code: ${country_code} | !Object.hasOwn: ${!Object.hasOwn(countryCoordinates, country_code)}`);
        }else {
            function sortByDistanceTo(coordsArray, referenceCoords) {
                const R = 6371e3; // Earth's radius in meters
                const radianConversion = Math.PI / 180;
                const [lat1, lon1] = referenceCoords;

                // calculate the distance between each coordinate and the reference coordinates
                const distances = coordsArray.map(coords => {
                    const [lat2, lon2] = coords.coordinates;

                    const dLat = (lat2 - lat1) * radianConversion;
                    const dLon = (lon2 - lon1) * radianConversion;
                    const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
                    Math.cos(lat1 * radianConversion) * Math.cos(lat2 * radianConversion) *
                    Math.sin(dLon / 2) * Math.sin(dLon / 2);
                    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
                    return R * c;
                });

                // create an array of indices to sort the original array by distance
                const indices = Array.from(coordsArray.keys());
                indices.sort((a, b) => distances[a] - distances[b]);

                // use the sorted indices to create a new array of sorted coordinates
                const sortedCoords = indices.map(i => coordsArray[i]);

                return sortedCoords;
            }

            const filteredTurnServers = sortByDistanceTo(turnCoordinates, countryCoordinates[country_code]).slice(0, 3)

            filteredTurnServers.map((turn) => {
                servers.iceServers[0].urls.push(...turn.ice.stun)
                servers.iceServers[1].urls.push(...turn.ice.turn)
            })
